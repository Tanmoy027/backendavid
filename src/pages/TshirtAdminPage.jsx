...
const TshirtAdminPage = () => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (editingItem && editingItem.image_path) {
          ...
          await storageService.deleteImage(editingItem.image_path, 'tshirt')
        }
        
        // Upload the new image
        console.log('Uploading image...')
        const uploadResult = await storageService.uploadImage(selectedFile, formData.title || 'tshirt', 'tshirt')
      ...
      if (editingItem) {
        console.log('Updating tshirt with data:', {
        ...
        await tshirtsService.updateTshirt(editingItem.id, {
          title: formData.title,
          description: formData.description,
          image_url: imageUrl,
          image_path: imagePath,
        ...
        // For new tshirts, require image upload
        if (!selectedFile) {
          alert('Please select an image file')
          setUploading(false)
          return
        ...
        console.log('Creating tshirt with data:', tshirtData)
        await tshirtsService.createTshirt(tshirtData)
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      // Find the tshirt to get its image path
      const tshirtToDelete = tshirts.find(item => item.id === id)
      
      // Delete the image from storage if it exists
      if (tshirtToDelete && tshirtToDelete.image_path) {
        console.log('Deleting image from storage:', tshirtToDelete.image_path)
        await storageService.deleteImage(tshirtToDelete.image_path, 'tshirt')
      }
      
      // Delete the tshirt record from the database
      await tshirtsService.deleteTshirt(id)
      loadTshirts()
    } catch (error) {
    ...
    <div className="admin-panel">
      ...
      <main className="admin-content">
        <div className="content-header">
          <h2>Manage T-Shirt Portfolio</h2>
          <button 
            onClick={() => {
              setEditingItem(null);
              setFormData({
          ...
          <div className="modal" onClick={(e) => e.target.className === 'modal' && resetForm()}>
            <div className="modal-content">
              <div className="modal-header">
                <h3>{editingItem ? 'Edit' : 'Add'} T-Shirt</h3>
...
export default TshirtAdminPage